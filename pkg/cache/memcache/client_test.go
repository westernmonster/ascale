	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				item: &Item{Key: "Test_client_Set", Value: []byte("abc")},
			},
			wantErr: false,
		},
	}
...
func Test_client_Add(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "add not exist value",
			args:    args{c: context.Background(), item: &Item{Key: key, Value: []byte("abc")}},
			wantErr: false,
		},
		{
			name:    "add exist value",
			args:    args{c: context.Background(), item: &Item{Key: key, Value: []byte("abc")}},
			wantErr: true,
		},
	}
...
func Test_client_Replace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "exist value",
			args:    args{c: context.Background(), item: &Item{Key: ekey, Value: []byte("abc")}},
			wantErr: false,
		},
	}
	for _, tt := range tests {
...
func Test_client_CompareAndSwap(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				c:    context.Background(),
				item: &Item{Key: ekey, cas: cas, Value: []byte("abc")},
			},
			wantErr: false,
		},
	}
...
func Test_client_Get(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "exist value",
			args:    args{c: context.Background(), key: ekey},
			wantErr: false,
			want:    "old",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var res string
			if err := testClient.Get(tt.args.c, tt.args.key).Scan(&res); (err != nil) != tt.wantErr ||
				res != tt.want {
				t.Errorf(
					"client.Get() = %v, want %v, got err: %v, want err: %v",
					err,
					tt.want,
					err,
					tt.wantErr,
...
func Test_client_Touch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "not exist value",
			args:    args{c: context.Background(), key: key, timeout: 100000},
			wantErr: true,
		},
		{
			name:    "exist value",
			args:    args{c: context.Background(), key: ekey, timeout: 100000},
			wantErr: false,
		},
	}
	for _, tt := range tests {
...
func Test_client_Increment(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "not exist value",
			args:    args{c: context.Background(), key: key, delta: 10},
			wantErr: true,
		},
		{
			name:         "exist value",
			args:         args{c: context.Background(), key: ekey, delta: 10},
			wantErr:      false,
			wantNewValue: 11,
		},
	}
