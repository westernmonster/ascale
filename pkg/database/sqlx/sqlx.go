...
import (
	...
	"ascale/pkg/conf/env"
	"ascale/pkg/database/sqlx/reflectx"
...
func (db *DB) Beginx(c context.Context) (t *Tx, err error) {
	now := time.Now()
	var span trace.Span
	if span = trace.SpanFromContext(c); span.IsRecording() {
		c, span = global.Tracer(env.AppID).Start(c, "trans")
...
func (db *DB) DBPing(ctx context.Context) (err error) {
	...
	if span = trace.SpanFromContext(ctx); span.IsRecording() {
		ctx, span = global.Tracer(env.AppID).Start(ctx, "ping")
